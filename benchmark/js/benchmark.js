...
async function runBenchy(gcode) {
...
runBenchyBtn.addEventListener('click', async function () {
  ...
  let benchyResults = Promise.all(
    [...Array(num)].map(async (_, it) => {
      result = await runBenchy(gcode);
